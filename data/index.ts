export const navItems = [
    { name: "About", link: "#about" },
    { name: "Projects", link: "#projects" },
    { name: "Skills", link: "#skills" },
    { name: "Contact", link: "#contact" },
  ];
  
  export const gridItems = [
    {
      id: 1,
      title: "I prioritize client collaboration, fostering open communication ",
      description: "",
      className: "lg:col-span-3 md:col-span-6 md:row-span-4 lg:min-h-[60vh]",
      imgClassName: "w-full h-full",
      titleClassName: "justify-end",
      img: "/b1.svg",
      spareImg: "",
    },
    {
      id: 2,
      title: "I'm very flexible with time zone communications",
      description: "",
      className: "lg:col-span-2 md:col-span-3 md:row-span-2",
      imgClassName: "",
      titleClassName: "justify-start",
      img: "",
      spareImg: "",
    },
    {
      id: 3,
      title: "My tech stack",
      description: "I constantly try to improve",
      className: "lg:col-span-2 md:col-span-3 md:row-span-2",
      imgClassName: "",
      titleClassName: "justify-center",
      img: "",
      spareImg: "",
    },
    {
      id: 4,
      title: "Tech enthusiast with a passion for development.",
      description: "",
      className: "lg:col-span-2 md:col-span-3 md:row-span-1",
      imgClassName: "",
      titleClassName: "justify-start",
      img: "/grid.svg",
      spareImg: "/b4.svg",
    },
  
    {
      id: 5,
      title: "Currently building cool 3D Projects",
      description: "The Inside Scoop",
      className: "md:col-span-3 md:row-span-2",
      imgClassName: "absolute right-0 bottom-0 md:w-96 w-60",
      titleClassName: "justify-center md:justify-start lg:justify-center",
      img: "/b5.svg",
      spareImg: "/grid.svg",
    },
    {
      id: 6,
      title: "Do you want to start a project together?",
      description: "",
      className: "lg:col-span-2 md:col-span-3 md:row-span-1",
      imgClassName: "",
      titleClassName: "justify-center md:max-w-full max-w-60 text-center",
      img: "",
      spareImg: "",
    },
  ];
  
  export const projects = [
    {
      id: 1,
      title: "Background Remover",
      des: "The Background Remover WebApp is a powerful and user-friendly tool designed to help users easily remove backgrounds from images with just a few clicks.",
      img: "/Bgs.png",
      iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/three.svg", "/fm.svg"],
      link: "https://github.com/harshdev2909/Background-remover",
      live:"https://background-remover-ten.vercel.app/",
    },
    {
      id: 2,
      title: "Chain-Org",
      des: "AI-powered conversational and generative AI platform, designed to enhance natural language processing capabilities.",
      img: "/gemini.png",
      iconLists: ["/next.svg", "/tail.svg", "/ts.svg", "/stream.svg", "/c.svg"],
      link: "",
      live:"",
    },
    {
      id: 3,
      title: "Echobeats - Music Application",
      des: "Echobeats is a sleek, modern music web application built using React and integrated with the Spotify API.",
      img: "/echobeats.png",
      iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/three.svg", "/c.svg"],
      link: "",
      live:"",
    },
    {
      id: 4,
      title: "Ecommerce-WebApp",
      des: "A fullstack E-commerce Web Application using React and express along with MongoDb.",
      img: "/ecom.jpg",
      iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/three.svg", "/c.svg"],
      link: "",
      live:"",
    },
    {
      id: 5,
      title: "Fhub-Food Delivery App",
      des: "This Xfood-hub WebApp is a full-stack application developed using the MERN (MongoDB, Express, React, Node.js) stack. The app provides a seamless user experience for browsing menus, placing orders, and tracking deliveries. Built with scalability and performance in mind, it is designed to handle the operations of a modern food delivery platform.",
      img: "https://wrapbootstrap.com/assets/items/thumb/8ea1a870b2c2998368606406c43b717a5f71f75db7df31211b6602024ae15644.webp?v=1710580878",
      iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/three.svg", "/c.svg"],
      link: "",
      live:"",
    },
    {
      id: 6,
      title: "X-movies",
      des: "This Xmovies WebApp is a full-stack application developed using the MERN (MongoDB, Express, React, Node.js) stack. It allows users to seamlessly browse movies, book tickets, and manage their bookings. The app offers a user-friendly interface and robust functionality, providing a complete end-to-end movie ticket booking experience.",
      img: "https://w3layouts.com/wp-content/uploads/2020/08/Pro-Showz-Website-Template-1-scaled.jpg",
      iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/three.svg", "/c.svg"],
      link: "",
      live:"",
    },
    {
      id: 7,
      title: "Cafe91",
      des: "This Harsh-Café WebApp is a modern, visually appealing frontend application built using React, Tailwind CSS, and Framer Motion. The app provides a delightful digital experience for browsing and ordering from a coffee menu. It incorporates smooth animations, clean design, and responsive layout, making it easy for users to explore various coffee .",
      img: "https://themewagon.com/wp-content/uploads/2018/07/Coffee-free-coffee-shop-bootstrap-template-1.jpg",
      iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/three.svg", "/c.svg"],
      link: "",
      live:"",
    },
    {
      id: 8,
      title: "UniVibe",
      des: "Build a modern social app with a stunning UI with a native mobile feel, a special tech stack, an infinite scroll feature, and amazing performance using React JS, Appwrite, TypeScript, and more.",
      img: "https://hasthemes.com/blog/wp-content/uploads/2022/01/Sociala-Social-Network-App-HTML-Template.png",
      iconLists: ["/re.svg", "/tail.svg", "/ts.svg", "/three.svg", "/c.svg"],
      link: "",
      live:"",
    },
  ];
  
  export const testimonials = [
    {
      quote:
        "Collaborating with Adrian was an absolute pleasure. His professionalism, promptness, and dedication to delivering exceptional results were evident throughout our project. Adrian's enthusiasm for every facet of development truly stands out. If you're seeking to elevate your website and elevate your brand, Adrian is the ideal partner.",
      name: "Michael Johnson",
      title: "Director of AlphaStream Technologies",
    },
    {
      quote:
        "Collaborating with Adrian was an absolute pleasure. His professionalism, promptness, and dedication to delivering exceptional results were evident throughout our project. Adrian's enthusiasm for every facet of development truly stands out. If you're seeking to elevate your website and elevate your brand, Adrian is the ideal partner.",
      name: "Michael Johnson",
      title: "Director of AlphaStream Technologies",
    },
    {
      quote:
        "Collaborating with Adrian was an absolute pleasure. His professionalism, promptness, and dedication to delivering exceptional results were evident throughout our project. Adrian's enthusiasm for every facet of development truly stands out. If you're seeking to elevate your website and elevate your brand, Adrian is the ideal partner.",
      name: "Michael Johnson",
      title: "Director of AlphaStream Technologies",
    },
    {
      quote:
        "Collaborating with Adrian was an absolute pleasure. His professionalism, promptness, and dedication to delivering exceptional results were evident throughout our project. Adrian's enthusiasm for every facet of development truly stands out. If you're seeking to elevate your website and elevate your brand, Adrian is the ideal partner.",
      name: "Michael Johnson",
      title: "Director of AlphaStream Technologies",
    },
    {
      quote:
        "Collaborating with Adrian was an absolute pleasure. His professionalism, promptness, and dedication to delivering exceptional results were evident throughout our project. Adrian's enthusiasm for every facet of development truly stands out. If you're seeking to elevate your website and elevate your brand, Adrian is the ideal partner.",
      name: "Michael Johnson",
      title: "Director of AlphaStream Technologies",
    },
  ];
  
  export const companies = [
    {
      id: 1,
      name: "cloudinary",
      img: "/cloud.svg",
      nameImg: "/cloudName.svg",
    },
    {
      id: 2,
      name: "appwrite",
      img: "/app.svg",
      nameImg: "/appName.svg",
    },
    {
      id: 3,
      name: "HOSTINGER",
      img: "/host.svg",
      nameImg: "/hostName.svg",
    },
    {
      id: 4,
      name: "stream",
      img: "/s.svg",
      nameImg: "/streamName.svg",
    },
    {
      id: 5,
      name: "docker.",
      img: "/dock.svg",
      nameImg: "/dockerName.svg",
    },
  ];
  
  export const workExperience = [
    {
      id: 1,
      title: "Frontend Developer Intern",
      desc: "Assisted in the development of a web-based platform using React.js, enhancing interactivity.",
      className: "md:col-span-2",
      thumbnail: "/exp1.svg",
    },
    {
      id: 2,
      title: "Core Community-Lead",
      desc: "Organised Various Tech events and hackathon with hackwithindia.",
      className: "md:col-span-2", // change to md:col-span-2
      thumbnail: "/exp2.svg",
    },
    {
      id: 3,
      title: "Freelance Web Dev Project",
      desc: "Led the dev of a WebApp app for a client, from initial concept to deployment on Web.",
      className: "md:col-span-2", // change to md:col-span-2
      thumbnail: "/exp3.svg",
    },
    {
      id: 4,
      title: "Lead Frontend Developer",
      desc: "Developed and maintained user-facing features using modern frontend technologies.",
      className: "md:col-span-2",
      thumbnail: "/exp4.svg",
    },
  ];
  
  export const socialMedia = [
    {
      id: 1,
      img: "/git.svg",
    },
    {
      id: 2,
      img: "/twit.svg",
    },
    {
      id: 3,
      img: "/link.svg",
    },
  ];
 export const skills = [
  {
    id:1,
    title: "HTML5",
    thumbnail: "https://img.icons8.com/color/96/000000/html-5.png",
  },
  {
      title: "CSS3",
      thumbnail: "https://img.icons8.com/color/96/000000/css3.png"
  },
  {
      title: "JavaScript",
      thumbnail: "https://img.icons8.com/color/96/000000/javascript--v1.png"
  },
  {
      title: "React.js",
      thumbnail: "https://img.icons8.com/color/96/000000/react-native.png"
  },
  {
      title: "Node.js",
      thumbnail: "https://img.icons8.com/color/96/000000/nodejs.png"
  },
  {
      title: "Express.js",
      thumbnail: "https://img.icons8.com/fluency/48/express-js.png"
  },

  {
      title: "MongoDB",
      thumbnail: "https://img.icons8.com/color/96/000000/mongodb.png"
  },

  {
      title: "Bootstrap",
      thumbnail: "https://img.icons8.com/color/96/000000/bootstrap.png"
  },
  {
      title: "Next.js",
      thumbnail: "https://img.icons8.com/color/48/nextjs.png"

  },
  {
      title: "Firebase",
      thumbnail: "https://img.icons8.com/color/96/000000/firebase.png"
  },
  {
      title: "Figma",
      thumbnail: "https://img.icons8.com/color/96/000000/figma--v1.png"
  },
  {
      title: "GSAP",
      thumbnail: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT_LoGVASma67wDkLNZB4a2XKUPyAGa1gmlBg&usqp=CAU"
  },
  {
      title: "Framer Motion",
      thumbnail: "data:image/jpeg;base64,"
  },
  {
    title: "Material-UI",
    thumbnail: "https://img.icons8.com/color/96/000000/material-ui.png"
},
{
    title: "Tailwind CSS",
    thumbnail: "https://img.icons8.com/color/96/000000/tailwindcss.png"
}
 ]